...
bot.on("message", message => {
  ...
  if (
    (message.content.toLowerCase().search(/alco+l/g) > -1 &&
      message.author.id !== bot.user.id) ||
    (message.content.toLowerCase().search(/bière/g) > -1 &&
      message.author.id !== bot.user.id) ||
    ...
    (message.content.toLowerCase().search(/vodka/g) > -1 &&
      message.author.id !== bot.user.id) ||
    (message.content.toLowerCase().search(/ vin/g) > -1 &&
      message.author.id !== bot.user.id) ||
    (message.content.toLowerCase().search(/rhum/g) > -1 &&
      message.author.id !== bot.user.id) ||
    (message.content.toLowerCase().search(/ bu/g) > -1 &&
      message.author.id !== bot.user.id) ||
    (message.content.toLowerCase().search(/gin/g) > -1 &&
      message.author.id !== bot.user.id) ||
    (message.content.toLowerCase().search(/ricard/g) > -1 &&
      message.author.id !== bot.user.id) ||
    (message.content.toLowerCase().search(/boire/g) > -1 &&
      message.author.id !== bot.user.id)
  ) {
  ...
  if (
    ...
    switch (cmd) {
      case "prison":
        if (args.length === 0) {
          message.channel.send(
            `Vous devez mentionner un utilisateur à mettre en prison, ${authorMess}.`
          );
        } else {
          let votedUser = message.mentions.users.first();
          if (votedUser === undefined) {
            message.channel.send(
              `Vous devez mentionner quelqu'un de ce salon, pardi ${authorMess} !`
            );
          } else {
            ...
            if (auth.auth_ids.includes(votedUser.id)) {
              message.channel
                .send(
                  `On n'envoie pas le shérif en prison, cowboy ${authorMess} ...`
                )
                .then(mess => {
                  mess.guild.members
                    .get(authorMess.id)
                  ...
                  authorMess.send(
                    "Bienvenue en prison... #lataule est disponible pour pleurer. Tu peux jouer **ici** au casino (en tapant ***!casino***) pour tenter de sortir."
                  );
                });
              ...
              fs.readFile("tries.json", "utf8", (err, data) => {
                ...
                if (
                  ...
                  message.channel
                    .send(
                      `T'as déjà fait appel au jury aujourd'hui ${authorMess}, non ? Prison pour délation !`
                    )
                    .then(mess => {
                      mess.guild.members
                        .get(authorMess.id)
      ...
      case "free":
        if (args.length === 0) {
          message.channel.send(
            `Vous devez mentionner un utilisateur à libérer de prison, ${authorMess}.`
          );
        } else {
          let votedUser = message.mentions.users.first();
      ...
      case "niquetoi":
        ...
        if (auth.auth_ids.includes(authorMess.id)) {
          if (votedUser !== undefined) {
            message.channel
              .send(`Dégage en taule, pourriture communiste ${votedUser} ...`)
              .then(mess => {
                mess.guild.members
                  .get(votedUser.id)
                  .addRole(mess.guild.roles.find(x => x.name === rolePrison));
          ...
          message.channel.send(
            `Tu t'es cru shérif ${message.author.username} ou quoi ?`
          );
        }
    }
  }
  ...
  if (event.t === "MESSAGE_REACTION_ADD") {
    ...
    let message = channel.fetchMessage(event.d.message_id).then(msg => {
      ...
      if (
        ...
        let countThumbsUp = msg.reactions
          .filter(reaction => reactionsArray[0] === reaction.emoji.name)
          .map(reaction => reaction.count)[0];
        let countThumbsDown = msg.reactions
          .filter(reaction => reactionsArray[1] === reaction.emoji.name)
          .map(reaction => reaction.count)[0];
        ...
        if (votedUser.id !== bot.user.id) {
          ...
          if (countThumbsUp >= requiredVotings) {
            msg.guild.members
              .get(votedUser.id)
              .addRole(msg.guild.roles.find(x => x.name === rolePrison))
              .then(() => {
                // récup de la durée du ban
                channel.fetchMessage(event.d.message_id).then(mg => {
                  let prisonTime =
                    re_duree.exec(mg.content) !== undefined
                      ? re_duree.exec(mg.content).groups.duree
                      : defaultPrisonTime;
                  ...
                  channel.send({
                    embed: {
                      ...
                      image: {
                        url:
                          "https://66.media.tumblr.com/22b23291747319d693a2e8d029b1c59a/tumblr_o5afscBOLE1rb2l1co1_400.gif"
                      }
                    }
                  });
                  channel
                    .send(
                      `***@everyone*** : ${votedUser}, les membres de La Piscine ont décidé de vous éliminer, et leur sentence est irrévocable. (enfin ... pendant ` +
                        prisonTime +
                        ` minutes.)`
                    )
                    .then(() => {
              ...
              mg.delete().then(() => {
                channel.send({
                  embed: {
                    ...
                    image: {
                      url:
                        "https://66.media.tumblr.com/1517c2ab08e08ffb2ad96dc2af3fedea/tumblr_nnn9o1mZJQ1rb2l1co1_500.gif"
                    }
                  }
                });
        ...
        if (votedUser && votedUser.id !== bot.user.id) {
          ...
          if (countThumbsUp >= requiredVotings) {
            channel.fetchMessage(event.d.message_id).then(mg => {
              msg.guild.members
                .get(votedUser.id)
                .removeRole(msg.guild.roles.find(x => x.name === rolePrison))
                .then(() => {
                  channel.send({
                    embed: {
                      color: 3447003,
                      image: {
                        url:
                          "https://66.media.tumblr.com/02c5ec43344c6ce200f8ecf102bc23c7/tumblr_nnrypiSVwo1rb2l1co1_500.gif"
                      }
                    }
                  });
                  channel.send(
                    `***@everyone*** : ${votedUser} est sorti(e) de prison. Attention à vos yeux.`
                  );
                  mg.delete();
                });
            });
          ...
          if (countThumbsDown >= requiredVotings) {
            channel.fetchMessage(event.d.message_id).then(mg => {
              mg.delete().then(() => {
                channel.send(
                  `***@everyone*** : ${votedUser} reste à croupir en taule.`
                );
              });
            });
          }
...
actions.startGame = async function(mr, user) {
  ...
  if (
    mr.count >=
    _local.lobby.players.length +
      1 /*mr.count >= Math.ceil(_local.lobby.players.length / 2) + 1*/
  ) {
    startGame();
  }
};
...
actions.triggerFlag = async function(mr, user) {
  ...
  if (_local.lobby.inGame(userx)) {
    ...
    if (
      _local.lobby.players.length === 0 &&
      _local.temp.timeoutOpt !== undefined
    ) {
      // Clear TO
    ...
    if (_local.lobby.join(userx) === false) {
      user.send(
        "La partie est pleine, désolé. Tente une prochaine fois (ou vire un designer)"
      );
    } else {
      addPlayer(userx);
      ...
      if (_local.lobby.players.length === 3) {
        _local.temp.pycx = await channel.send(
          "Réagissez à ce message pour commencer la partie. [Tout le monde doit voter pour commencer]"
        );
      ...
      if (_local.lobby.players.length === 1) {
        _local.temp.timeoutOpt = setTimeout(function() {
          for (var i = 0; i < _local.lobby.players.length; i++) {
            channel.members
              .get(_local.lobby.players[i])
              .send(
                "Hey... J'ai du relancer le Limite Limite parce qu'il était lancé depuis trop longtemps. Rejoingez de nouveau et relancez si vous voulez jouer."
              );
          }
          reset();
        }, 10 * 60 * 1000);
...
bot.on("messageReactionAdd", function(mr, user) {
  ...
  for (var i = 0; i < _local.reactables.length; i++) {
    if (
      _local.reactables[i].message.id === message.id &&
      _local.reactables[i].emote === mr.emoji.toString()
    ) {
...
async function addPlayer(userx) {
  var channel = bot.channels.get(config["active-channel"]);
  var role = channel.guild.roles.find(
    x => x.name === config["roles"]["player"]
  );
...
async function sendIntro() {
  ...
  var attachment = new Discord.Attachment(
    fs.readFileSync(path.join(__dirname, "assets/banner.png")),
    "banner.png"
  );

  await channel.send(attachment);
  var react_to = await channel.send(
    "**Limite Limite**\n\n*Réagisser à ce message pour jouer.*"
  );

  var current_players =
    _local.lobby.players.join(", ") === ""
      ? " "
      : _local.lobby.players.join(", ");

  _local.temp.pycm = await channel.send(
    "Joueurs actifs :\n```" + current_players + "```"
  );
...
async function removeReactables(
  message,
  clearReactions = false,
  deleteMessage = false
) {
...
async function round() {
  ...
  channel.send(
    "**" + fetchUserX(actions.czar) + "** est le Boss pour cette manche"
  );
  ...
  for (var i = 0; i < game.players.length; i++) {
    if (game.players[i].id !== actions.czar) {
      var message =
        "Tu as `" + game.players[i].cards.length + "` cartes réponses.";

      if (actions.pick > 1) {
        var concatable =
          "\nChoisis " +
          actions.pick +
          " cartes en entrant leurs numéros séparés par une virgule*.";
      } else {
      ...
      picks.push({
        ...
        promise: dmc.awaitMessages(filter, { max: 1, time: 60 * 1000 })
      });
    }
  }
  ...
  for (var i = 0; i < picks.length; i++) {
    ...
    colx =
      colx !== undefined
        ? colx.content.split(",").map(x => x.trim())
        : new Array();
    ...
    if (colx.length < 1) {
      channel.members
        .get(picks[i].id)
        .user.send(
          "T'as pas répondu à temps, alors j'ai choisi une carte au hasard pour toi. :yum:"
        );
...
async function czarPick(czar) {
  ...
  for (var i = 0; i < presentables.length; i++) {
    // Await
    await sendReactableCard(
      channel,
      presentables[i].response,
      [255, 255, 255],
      "✅",
      presentables[i].id
    );
  ...
  function warnAndKick() {
    _local.temp.timeoutWarn = setTimeout(function() {
      channel.send(
        "!!! **<@" +
          czar +
          ">, tu as " +
          Math.round((timeout - 0.75 * timeout) / 1000) +
          " secondes restantes pour choisir une carte, sinon j't'envoie en prison** !!!"
      );
    }, 0.75 * timeout);
    ...
    _local.temp.timeoutKick = setTimeout(function() {
      ...
      channel.send(
        "**" +
          fetchUserX(czar) +
          " a été banni du jeu pour être un flemmard trop lent à choisir !**."
      );
  ...
  async function sendReactableCard(channel, contents, colour, reaction, id) {
    ...
    async function done(mx) {
      ...
      channel.send(
        "**" + fetchUserX(winner.id) + "** a gagné la manche ! " + wittyQuote()
      );
  ...
  async function clearReactionsAndEdit() {
    for (var i = 0; i < _local.temp.czarPicks.length; i++) {
      ...
      _local.temp.czarPicks[i].message.edit(
        makeMultiCard(
          channel,
          _local.temp.czarPicks[i].contents,
          [255, 255, 255],
          _local.temp.czarPicks[i].id
        )
      );
    }
...
async function verifyWin() {
  ...
  if (verif.win) {
    ...
    if (verif.true_win) {
      ...
      await channel.send(
        "**" +
          toUserX(verif.winner.map(x => x.id)).join(", ") +
          "** a gagné la partie (mais bon, c'est juste parce que tout le monde dormait ou a été kick en prison). " +
          wittyQuote()
      );
    }
...
function delayedReset(channel, time = 2 * 60 * 1000) {
  channel.send(
    "Le channel est détruit dans " + Math.round(time / 1000) + " secondes."
  );
...
function wittyQuote() {
  var wits = fs
    .readFileSync(
      __dirname + "/" + config["directories"]["witty-quotes"],
      "utf8"
    )
    .split("\n")
    .filter(x => x.length > 0);
...
function adventOfCode(user) {
  user
    .send({
      embed: {
        color: 3447003,
        title: "Bienvenue en taule !",
        description:
          "Heureusement, tu peux sortir plus tôt. Joue au casino. Si tu gagnes, tu sors."
      }
    })
    .then();
}
